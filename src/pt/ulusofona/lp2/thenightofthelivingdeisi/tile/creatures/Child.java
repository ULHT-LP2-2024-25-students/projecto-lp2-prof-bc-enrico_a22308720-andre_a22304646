...
public class Child extends Creature {
    ...
    public Child(int[] positionInBoard, int id, int team, String name, State state, int type) {
    ...
    public TypeMove getTypeMove(Tile tile, Tile tileDestiny){
        if(tileDestiny.getCreature() == null){
            ...
            if (tileDestiny.getCreature().getState() != State.LIVE){
                if (this.equipment!=null){
                    return this.equipment.canDefend() ? TypeMove.PASS : TypeMove.INVALID;
                }
            }
        }else if (this.state != State.LIVE){
